...
NSString *const kGPUImageIGAntiLuxFragmentShaderString = SHADER_STRING
(
...
//     vec4 texel = texture2D(s_texture, sourceTextureCoordinate);
     ...
     vec3 blurredTexel = texture2D(inputImageTexture3, textureCoordinate).rgb;
...
//     float flatBright = histogramCenter(cdf, textureCoordinate, hsv.z);
     float flatBright = histogramCenter(inputImageTexture2, textureCoordinate, hsv.z);
...
@implementation RCIGAntiLuxFilter
@synthesize filterStrength = _filterStrength;

- (id)init
{
    if (!(self = [super initWithFragmentShaderFromString:kGPUImageIGAntiLuxFragmentShaderString]))
    {
        return nil;
    }
    
    filterStrengthUniform = [filterProgram uniformIndex:@"filterStrength"];
...
{
    _filterStrength = newValue;
    [self setFloat:_filterStrength forUniform:filterStrengthUniform program:filterProgram];
}
@end