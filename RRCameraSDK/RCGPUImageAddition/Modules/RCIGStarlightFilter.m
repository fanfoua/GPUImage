...
NSString *const kGPUImageIGStarlightFragmentShaderString = SHADER_STRING
(
...
//     vec4 texel = texture2D(s_texture, sourceTextureCoordinate);
     ...
     hsv.z = clahe2D(inputImageTexture2, textureCoordinate, hsv.z);
//     hsv.z = clahe2D(cdf, sourceTextureCoordinate, hsv.z);
...
@implementation RCIGStarlightFilter
@synthesize filterStrength = _filterStrength;

- (id)init
{
    if (!(self = [super initWithFragmentShaderFromString:kGPUImageIGStarlightFragmentShaderString]))
    {
        return nil;
    }
