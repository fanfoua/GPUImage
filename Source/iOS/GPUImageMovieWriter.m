...
- (void)setupAudioReaderWithTracks:(NSMutableArray *)audioTracks {
    [self setupAudioReaderWithTracks:audioTracks duration:kCMTimeZero];
}

- (void)setupAudioReaderWithTracks:(NSMutableArray *)audioTracks duration:(CMTime)duration {
    if(audioTracks.count > 0){
        ...
        for(AVAssetTrack *track in audioTracks){
            ...
            [compositionCommentaryTrack insertTimeRange:CMTimeRangeMake(kCMTimeZero, CMTimeCompare(duration, kCMTimeZero) ? duration : track.asset.duration)
                                                ofTrack:track
...
- (void)startAudioWritingWithComplectionBlock:(void (^)())onCompletionBlock {
    if(assetAudioReader){
